...
model_forward <- regsubsets(logprice ~ ., data = diamonds3, method = "forward") # forward stepwise selection
model_backward <- regsubsets(logprice ~ ., data = diamonds3, method = "backward") # backward stepwise selection
...
which.max(summary(model_backward)$adjr2)
plot(summary(model_backward)$adjr2, xlab = "Number of Variables", ylab = "Adjusted R^2", type = "b")

# Using adjusted R^2 as criterion the best subset from a backward stepwise selection has 8 predictors.
